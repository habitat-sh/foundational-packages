...
$pkg_bin_dirs=@(
    ...
    "Contents\VC\Redist\MSVC\14.44.35112\arm64\Microsoft.VC143.CRT",
    "Contents\MSBuild\Current\Bin\arm64",
	...
	"Contents\VC\Tools\Llvm\ARM64\bin"
)
$pkg_lib_dirs=@(
    "Contents\VC\Tools\MSVC\14.44.35207\atlmfc\lib\arm64",
    "Contents\VC\Tools\MSVC\14.44.35207\lib\arm64"
)
$pkg_include_dirs=@(
...
function Invoke-Unpack {
    ...
    try {
        # We only want the ARM64 flavored vsix payloads for this (host+target) arm64 build;
        # exclude x86 and x64 rather than the (32-bit) arm ones.
        Get-ChildItem "$HAB_CACHE_SRC_PATH/$pkg_dirname" -Include *.vsix -Exclude @('*x86*', '*x64*') -Recurse | ForEach-Object {
            Rename-Item $_ "$_.zip"
