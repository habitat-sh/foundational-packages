...
$pkg_bin_dirs=@(
    ...
    "Windows Kits\10\bin\$pkg_version.0\arm64"
)
$pkg_lib_dirs=@(
    "Windows Kits\10\Lib\$pkg_version.0\um\arm64",
    "Windows Kits\10\Lib\$pkg_version.0\ucrt\arm64"
)
$pkg_include_dirs=@(
...
function Invoke-Unpack {
    Start-Process "$HAB_CACHE_SRC_PATH/$pkg_filename" -Wait -ArgumentList "/features OptionId.DesktopCPPARM64 /quiet /layout $HAB_CACHE_SRC_PATH/$pkg_dirname"
    ...
    # We only want the arm64 arch-specific binaries/libs for this build; drop x86, x64, and 32-bit arm.
    Get-ChildItem "$HAB_CACHE_SRC_PATH/$pkg_dirname" -Include @("x86", "x64", "arm") -Recurse | ForEach-Object {
